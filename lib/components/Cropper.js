...
class Cropper extends React.Component {
  ...
  boundedCoords (x, y, dx, dy) {
    ...
    if (newX - dw > 0) { x = dw; }
    else if (imageRightEdge - dx < rightEdge) { x = x; }
    ...
    var topEdge = 0;
    if (newY - dw > 0) { y = dh; }
    else if (imageBottomEdge - dy < bottomEdge) { y = y; }
  ...
  render () {
    return (
      <div>
        <canvas ref="canvas" onMouseDown={this.mouseDownListener.bind(this)} width={this.props.width} height={this.props.height}></canvas>
        {this.state.preview &&
          <div>
